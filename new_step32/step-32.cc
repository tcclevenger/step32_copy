...
//Matrix Free Operator for velocity block (0,0)
...
public:
  ABlockOperator ();
  void clear ();
  void evaluate_eta_x_2(const double eta);
...
private:
  ...
  VectorizedArray<number> eta_x_2;
};
template <int dim, int degree_v, typename number>
...
//Copy between TrilinosWrappers::MPI::Vector and LinearAlgebra::distributed::Vector<double>
          ...
          const TrilinosWrappers::MPI::Vector &in)
{
  ...
  public:
               ...
               const TrilinosWrappers::MPI::BlockVector &src) const
    {
      ...
      LinearAlgebra::distributed::Vector<double> new_dst(partition,MPI_COMM_WORLD);
      LinearAlgebra::distributed::Vector<double> new_utmp(partition,MPI_COMM_WORLD);
      ChangeVectorTypes::copy(new_dst,dst.block(0));
      ChangeVectorTypes::copy(new_utmp,utmp);

      //std::cout << "utmp:    " << std::abs(new_utmp.l2_norm() - utmp.l2_norm()) << std::endl;
      ...
      if (do_solve_A == true)
      {
        ...
        a_preconditioner.vmult(new_dst, new_utmp);

      ChangeVectorTypes::copy(dst.block(0),new_dst);

      //std::cout << "dst:    " << std::abs(new_dst.norm_sqr() - dst.block(0).norm_sqr()) << std::endl;
    }
...
//                                                      mapping);
    ...
    // Now also set up the mf stuff
    setup_mf_dofs ();
  }


  template <int dim>
  void BoussinesqFlowProblem<dim>::setup_mf_dofs()
  {
    Assert(velocity_degree==parameters.stokes_velocity_degree,
           ExcMessage("I don't know how to get around having the velocity degree as a global variable "
                      "since I must define the mg_matrices in the constructor of this problem based on "
                      "a matrix type (ABlockOperator) which needs the velocity degree. Thus is ruins the "
                      "of even having a .prm file."));
    ...
    //  DoFTools::extract_locally_relevant_dofs(stokes_dof_handler,
    //                                          velocity_relevant_set);
    //  velocity_constraints.reinit(velocity_relevant_set);
    ...
    //  std::set<types::boundary_id> no_normal_flux_boundaries;
    //  no_normal_flux_boundaries.insert(1);
    //  VectorTools::compute_no_normal_flux_constraints(velocity_dof_handler,
    //                                                  0,
    //                                                  no_normal_flux_boundaries,
    //                                                  velocity_constraints,
    //                                                  mapping);
    ...
    mg_constrained_dofs.make_zero_boundary_constraints(velocity_dof_handler, dirichlet_boundary);

    for (unsigned int level=0; level<n_levels; ++level)
    {
      IndexSet relevant_dofs;
      DoFTools::extract_locally_relevant_level_dofs(velocity_dof_handler, level, relevant_dofs);
      ...
      std::shared_ptr<MatrixFree<dim,double> > mf_storage(new MatrixFree<dim,double>());
      mf_storage->reinit(velocity_dof_handler, level_constraints,
                         QGauss<1>(velocity_degree+1), additional_data);

      mg_matrices[level].clear();
      mg_matrices[level].initialize(mf_storage, mg_constrained_dofs, level);
      mg_matrices[level].evaluate_eta_x_2(EquationData::eta);
  ...
  {
    {
      ...
      for (unsigned int level = 0; level<triangulation.n_global_levels(); ++level)
      {
        if (level > 0)
        {
          ...
          //smoother_data[level].smoothing_range = 0.8;
      ...
      PreconditionMG<dim, vector_t, Transfer>
          precondition_gmg(velocity_dof_handler, mg, mg_transfer);
      ...
      try
      {
        const LinearSolvers::BlockSchurPreconditioner<
            PreconditionMG<dim, vector_t, Transfer>, //TrilinosWrappers::PreconditionAMG,
            TrilinosWrappers::PreconditionJacobi>
            preconditioner(stokes_matrix,
                           stokes_preconditioner_matrix,
                           *Mp_preconditioner,
                           precondition_gmg, // *Amg_preconditioner,
                           false,
